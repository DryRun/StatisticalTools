...
inputFileData = TFile("/cmshome/gdimperi/Dijet/CMSDIJETrepo/CMSSW_7_2_1_DiJet/src/CMSDIJET/DijetRootTreeAnalyzer/test_fit/toys_Bonly.root")


for itoy in range(1,11):

  inputFitRes = TFile("fit_studies/output_toys_root/mlfitToy"+str(itoy)+".root")
  hist_mass = inputFileData.Get("hist_mass_toy"+str(itoy))
  hist_binned = inputFileData.Get("hist_binned_toy"+str(itoy))
...
##END OF LOOP OVER TOYS  
  ...
  c.SaveAs(args.output+"/fit_Bonly_toy"+str(itoy)+".C")
  c.SaveAs(args.output+"/fit_Bonly_toy"+str(itoy)+".png")
  c.SaveAs(args.output+"/fit_Bonly_toy"+str(itoy)+".pdf")
  c.Clear()
