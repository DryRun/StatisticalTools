...
class MjjFit:
	...
	def fit(self, save_to, signal_name=None, fix_p3=False, fit_range=[300., 1200.], fit_strategy=1):
		...
		getattr(w,'import')(background_pdf,RooFit.Rename("background"))
		getattr(w,'import')(background_norm,ROOT.RooCmdArg())
		getattr(w,'import')(self.data_roohistogram_,RooFit.Rename("data_obs"))
		getattr(w, 'import')(model, RooFit.Rename("model"))
		if signal_name:
			getattr(w,'import')(signal_roohistogram,RooFit.Rename("signal"))
			getattr(w,'import')(signal_pdf,RooFit.Rename("signal_pdf"))
			getattr(w,'import')(signal_norm,ROOT.RooCmdArg())
	
		w.Print()
		w.writeToFile(save_to)
		if signal_name:
