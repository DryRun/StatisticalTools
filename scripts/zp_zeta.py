...
#masses = [400]
basedir = "/home/dryu/Dijets/data/EightTeeEeVeeBee/ZPrime/MG5_aMC_v2_5_5/scripts"
...
for mass in masses:
	...
	for name, initial_states in {"u":["u u~"], "d":["d d~"], "pp":["u u~", "d d~"]}.iteritems():
		...
		#	sys.exit(1)
		#width_file.close()
		width = (1.5**2 * mass * (5. + (1+2*mt**2/mass**2)*math.sqrt(max(0., 1 - (4*mt**2)/mass**2))))/(144.*math.pi)
		...
		if mass > 2 * mt:
			tpart = (1+2*mt**2/mass**2)*(1. - 4*mt**2/mass**2)**0.5
		else:
			tpart = 0.
